        ...
        <div class="card card-block">
          <form class="form-inline mb-3" method="get" action="">
            ...
            <div class="input-group datepicker-group" style="width:120px;display:inline-flex;">
              <input type="text" class="form-control date_input" name="start_date" value="<?= date('d-m-Y', strtotime($start_date)) ?>" placeholder="DD-MM-YYYY" maxlength="10">
            </div>
            &nbsp;&nbsp;
            <label>End Date</label>&nbsp;
            <div class="input-group datepicker-group" style="width:120px;display:inline-flex;">
              <input type="text" class="form-control date_input" name="end_date" value="<?= date('d-m-Y', strtotime($end_date)) ?>" placeholder="DD-MM-YYYY" maxlength="10">
            </div>&nbsp;

            <label>Show Deleted Journal</label>&nbsp;
            <div class="input-group datepicker-group" style="width:120px;display:inline-flex;">
              <input type="checkbox"   class="form-control" name="show_deleted" value="1" <?= isset($_GET['show_deleted']) && $_GET['show_deleted'] == '1' ? 'checked' : '' ?>>
            </div>&nbsp;
          ...
          <div class="row mb-2">
            <div class="col-md-3"  >
              ...
              <div class="input-group datepicker-group"   >
                <input type="text" style='width:120px;' name="journal_date" class="form-control date_input" value="<?= isset($_COOKIE['transaction_date']) && $_COOKIE['transaction_date'] ? date('d-m-Y', strtotime($_COOKIE['transaction_date'])) : date('d-m-Y') ?>" placeholder="DD-MM-YYYY" maxlength="10">
...
function addJournalLine(selectedAccountId = '') {
  ...
  const row = `<tr>
    <td>${index}</td>
    <td>
      <select name="category[]" class="form-control select2_account" style="width:550px;">
        ${options}
      </select>
    </td>
...
function updateTotals() {
 ...
 $(document).on('focus', '.contact_select', function () {
...
function deleteJournal(id) {
  ...
  $.post('journal/delete_journal.php', { id }, function(resp) {
...
function RestoreJournal(id) {
  ...
  $.post('journal/restore_journal.php', { id }, function(resp) {
...
function viewJournal(id) {
  ...
  $.get('journal/get_journal_details.php', { id }, function(res) {
    if (res && res.journal) {
      const j = res.journal;
          ...
          $('#deleteJournalBtn').on('click', function() {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                $.post('journal/delete_journal.php', { id: j.id }, function(resp) {
