...
$periods = [];
$q = mysqli_query($con, "
    SELECT id, perid_from, period_to, due_date, log_old_period, status 
              ...
              <table id="example" class="table table-bordered table-striped table-hover table-sm">
                ...
                <?php foreach ($periods as $p): ?>
                  ...
                  <tr>
                    ...
                    <td>
                      <button type="button" class="btn btn-sm btn-primary" onclick="loadEdit(<?= $p['id'] ?>);">
                        Edit
                      </button>

                      <button type="button" class="btn btn-sm btn-danger" onclick="deletePeriod(<?= $p['id'] ?>);">
                        Delete
                      </button>
                    </td>
                  </tr>
               ...
               <label class="col-md-3 col-form-label text-right">Period From</label>
                  <div class="col-md-3">
                     <input type="text" name="perid_from" class="form-control date_input" 
                           placeholder="dd/mm/yyyy" required value='<?= isset($next_day) ? date('d/m/Y', strtotime($next_day)) : '' ?>'>
                  </div>
                  ...
                  <div class="col-md-3">
                     <input type="text" name="period_to" class="form-control date_input" 
                           placeholder="dd/mm/yyyy" required value='<?= isset($next_year_end) ? date('d/m/Y', strtotime($next_year_end)) : '' ?>'>
                  </div>
                </div>
               <div class="form-group row align-items-center">
                  <!-- Due Date -->
                  ...
                  <div class="col-md-3">
                     <input type="text" name="due_date" class="form-control date_input" 
                           placeholder="dd/mm/yyyy" required value='<?= isset($next_year_end_due) ? date('d/m/Y', strtotime($next_year_end_due)) : '' ?>'>
                  </div>
...
function loadEdit(id) {
    $.post("ajax_accounting_period/load_single.php", {id:id}, function(res){
        if (res.success) {
            ...
            $('input[name=log_old_period]').prop('checked', res.data.log_old_period == 1);
...
function deletePeriod(id) {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      $.post("ajax_accounting_period/delete_period.php",{id:id},function(res){
        if (res.success) {
